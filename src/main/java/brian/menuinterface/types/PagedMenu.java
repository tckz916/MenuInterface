...
public class PagedMenu extends IMenu {
    ...
    public Inventory build() {
        ...
        List<PagedButton> pagedButtons = getButtons().stream().filter(button -> button instanceof PagedButton).map(button -> (PagedButton) button).collect(Collectors.toList());

        int pagesRequired = 0;
        int size = 0;

        if (pagedButtons.isEmpty()) try {
        ...
        if (getEmptySlots() % pagedButtons.size() == 0) {
            ...
            pagesRequired = 1;
        } else pagesRequired = Math.round(pagedButtons.size() / getEmptySlots());

        if((pagesRequired * getEmptySlots()) < pagedButtons.size()) pagesRequired++;
        ...
        for (PagedButton button : pagedButtons) {
            ...
            if (inventory.firstEmpty() != -1) inventory.addItem(button.getItem());
            else {

                currentPage++;
                pages.put(currentPage, Bukkit.createInventory(this, size, getTitle().replace("%current%", "" + currentPage).replace("%available%", "" + pagesRequired)));
    ...
    public Inventory getNextPage(Inventory clickedInventory) {
        ...
        if(pages.inverse().containsKey(clickedInventory)) return pages.get(pages.inverse().get(clickedInventory) + 1);
        return null;
