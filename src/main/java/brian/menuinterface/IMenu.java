...
public abstract class IMenu implements InventoryHolder, Listener {
     ...
     * @param inventory is a Inventory "_"
     */
    ...
    /**
     * @param rebuild sets a rule if the {@link IMenu#build()} needs to be called again or if {@link #inventory} is null
     * @return a Inventory
     */
     ...
     * @param button is a object that extends {@link IMenuButton}
     * @return a {@link IMenu}
     */
     ...
     * @param compare is a {@link ItemStack} that should be compared when looking for {@link IMenuButton}
     * @return a null or a {@link IMenuButton}
     */

    public IMenuButton findButtonByItem(ItemStack compare){return buttons.stream().filter(item -> item.equalsItem(compare) ).findFirst().orElse(null); }

    /**
     * Tries to find a {@link IMenuButton} by {@link IMenuButton#itemData} key
     * @param key is a map key that should be searched for in {@link IMenuButton#itemData}
     * @return a null or a {@link IMenuButton}
     */

    public IMenuButton findButtonByData(String key){ return buttons.stream().filter(item -> item.containsData(key)).findFirst().orElse(null); }

    /**
     * Tries to find a {@link IMenuButton} by {@link IMenuButton#getIdentifier()}
     * @param identifier is a {@link IMenuButton#getIdentifier()}
     * @return a null or a {@link IMenuButton}
     */

    public IMenuButton findButtonByIdentifier(String identifier){ return buttons.stream().filter(item -> item.getIdentifier().equalsIgnoreCase(identifier)).findFirst().orElse(null); }
    /**
     * Sets a parent of {@link IMenu}
     * @param menu is a parent of this {@link IMenu}
     * @return a {@link IMenu}
     */
    ...
    public IMenu getParent() { return parent; }

    /**
     * Assigns data to {@link #inventoryData}
     * @param key is a key for the map
     * @param object is a value for the map
     * @return a {@link IMenu}
     */

    public IMenu assignData(String key, Object object){ getInventoryData().put(key,object); return this; }

    /**
     * @return a {@link #inventoryData}
     */

    public Map<String, Object> getInventoryData() { return inventoryData; }

    /**
     * @param key is a key that will searched for in {@link #inventoryData}
     * @return a {@link Boolean} true if the key was found in {@link #inventoryData} or false if not.
     */

    public boolean containsData(String key){ return getInventoryData().containsKey(key); }
    ...
    /**
     * @param name of a child {@link IMenu}
    ...
    /**
     * @param rowsCount is a number not bigger than 6 as max inventory size is 54.
     * @return a {@link IMenu}
     */
    ...
    /**
     * @param title sets title of an {@link Inventory}
     */
    ...
    /**
     * @param design sets design of an menu {@link IMenu}
     * @return a {@link IMenu}
     */
    ...
    /**
     * @param inventory is where to place dummies
     * @return a {@link IMenu}
     */
    ...
    public IMenu setDummies(Inventory inventory){
        ...
        */
        for(IMenuButton button : getButtons().stream().filter(it -> it.isDummy() && it.isFiller()).collect(Collectors.toList())){
            if(button instanceof PagedButton) continue;
    ...
    /**
     * @param identifier is a identifier of a button in {@link #buttons}
     * @param inventory is a {@link Inventory}
     * @return a {@link IMenu}
     */
     ...
     * @param title is a string, make sure that it doesn't exceed minecraft limit which is 32
     * @return a {@link IMenu}
     */
    ...
    public IMenu changeTitle(String title){
        ...
        setTitle(title);
        getInventory().getViewers().forEach(player -> MenuUtil.changeTitle(((Player) player), title, "minecraft:chest"));
        return this;
