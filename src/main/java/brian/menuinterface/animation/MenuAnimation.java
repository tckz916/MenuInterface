...
public class MenuAnimation {
    ...
    public static MenuAnimation create() { return new MenuAnimation();}
     ...
     * @param frame is a frame position
     ...
     * @param designer is a design that will be used on that frame.
     * @return a {@link MenuAnimation}
     */
    public MenuAnimation setFrame(int frame, MenuDesigner designer){
        if(frames.containsKey(frame)) frames.remove(frame);
     ...
     * @param plugin is a plugin that will be used to register task.
     * @param menu is a menu that will be affected by the Animation.
     */
            ...
            public void run() {
                ...
                if(frames.size() + 1 == currentFrame){
                    if(!loop) {
                        ...
                        if(animationEndConsumer != null) animationEndConsumer.accept(menu);
                        cancel();
                        ...
                        if(delayBetweenLoops != -1) currentDelay = delayBetweenLoops;

                    }
                }

                if(taskToDoEveryFrame != null) taskToDoEveryFrame.accept(menu);
     ...
     * @param animationEndConsumer is an event
     */
     ...
     * @param delayBetweenLoops is ant integer in ticks
     */
     ...
     * @param taskToDoEveryFrame is an event.
     */
