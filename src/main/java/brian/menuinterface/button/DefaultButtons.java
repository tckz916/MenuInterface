...
public enum DefaultButtons {
    ...
    /**
     * Open can be used in any {@link IMenu} and forwards who clicked to defined {@link IMenu}, if there's no page found, throws a {@link brian.menuinterface.exceptions.PageNotFoundException}
     */

    OPEN("open", event -> {

        String nameOfInventory = event.getClickedButton().getItemData().get(DefaultButtons.OPEN.getIdentifier()).toString();
    ...
    /**
     * Return can be used in any {@link IMenu} and forwards to parent of clicked menu, if there's no parent found throws a {@link brian.menuinterface.exceptions.ParentNotFoundException}
     */
    ...
    RETURN("return", event -> {
        ...
        } else event.getWhoClicked().sendMessage(MenuLang.PAGE_NOT_FOUND.getLang().replace("%name%", "null"));
    });
    ...
    /**
     *
     * @return a {@link Consumer<ButtonClickEvent>} or in other words Button task {@link ButtonTask}
     */
    ...
    /**
     * @param identifier is the buttons identifier
     * @param consumer is a {@link Consumer<ButtonClickEvent>} or in other words Button task {@link ButtonTask}
     */
     ...
     * @param identifier is the buttons identifier
     * @param consumer is a {@link Consumer<ButtonClickEvent>} or in other words Button task {@link ButtonTask}
     * @param initializer is a initializer which will be ran on {@link IMenuButton} constructor
     */
    ...
    /**
     * @param item a {@link ItemStack} which is set when {@link IMenu} is building...
     * @return a {@link MenuButton}
     */

    public MenuButton getButtonOfItemStack(ItemStack item) {return new MenuButton(item,-1, this);}
    public MenuButton getButtonOfItemStack(ItemStack item, Object value) {return new MenuButton(item,-1, this,value);}
